...
public class GameManager {
    ...
    private FileHandle fileHandle = Gdx.files.local("bin/GameData.json"); // files.internal are read only, local can be written
    ...
    // We encode game data so the user cant change scores in the json file

    public void saveData(){
        if(gameData != null){
            fileHandle.writeString(Base64Coder.encodeString(json.prettyPrint(gameData)), false); // append false overwrites the data instead of adding
        }
    }
...
} // game manager, this is a singleton
